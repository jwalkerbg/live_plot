...
class LivePlot(QtWidgets.QMainWindow):
    ...
    def update_legend_values(self):
        """Update legend with color hints, markers, and Δ values (stable, draggable)."""
        ...
        if not hasattr(self, 'legend_text'):
            ...
            self.legend_text.setFlag(self.legend_text.ItemIsMovable)  # allow dragging
            self.plot_widget.getViewBox().addItem(self.legend_text, ignoreBounds=True)

            # Set initial position once
            vb = self.plot_widget.getViewBox()
            pos = vb.mapSceneToView(pg.QtCore.QPointF(10, 10))
            self.legend_text.setPos(pos.x(), pos.y())
        else:
            # Update text only; keep current position for dragging
