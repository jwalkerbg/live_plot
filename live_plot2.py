...
class LivePlot(QtWidgets.QMainWindow):
    ...
    def update_legend_values(self):
        """Update legend with color hints, markers, and Δ values (stable single TextItem)."""
        idx1 = int(self.marker1.value())
        idx2 = int(self.marker2.value())
        max_idx = len(self.data_a) - 1

        a1 = self.data_a[idx1] if 0 <= idx1 <= max_idx else float('nan')
        b1 = self.data_b[idx1] if 0 <= idx1 <= max_idx else float('nan')
        a2 = self.data_a[idx2] if 0 <= idx2 <= max_idx else float('nan')
        b2 = self.data_b[idx2] if 0 <= idx2 <= max_idx else float('nan')

        dx = idx2 - idx1
        dy_a = a2 - a1
        dy_b = b2 - b1

        # Build HTML text with colored squares
        legend_html = (
            f"<span style='color:magenta;'>■</span> Marker1 Index: {idx1}<br>"
            f"<span style='color:yellow;'>■</span> sin: {a1:.4f}  "
            f"<span style='color:cyan;'>■</span> cos: {b1:.4f}<br>"
            f"<span style='color:red;'>■</span> Marker2 Index: {idx2}<br>"
            f"<span style='color:yellow;'>■</span> sin: {a2:.4f}  "
            f"<span style='color:cyan;'>■</span> cos: {b2:.4f}<br>"
            f"ΔX: {dx}  Δsin: {dy_a:.4f}  Δcos: {dy_b:.4f}"
        )

        # Single TextItem anchored top-left, ignoreBounds prevents autoscale
        if not hasattr(self, 'legend_text'):
            self.legend_text = pg.TextItem(html=legend_html, anchor=(0, 0))
            self.plot_widget.getViewBox().addItem(self.legend_text, ignoreBounds=True)
        else:
            self.legend_text.setHtml(legend_html)

        # Position at top-left corner in scene coordinates
        vb = self.plot_widget.getViewBox()
        pos = vb.mapSceneToView(pg.QtCore.QPointF(30, 35))  # 10 px margin
